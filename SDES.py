...
class SDES :
    ...
    def generate_subkeys(cls, key) :
        ...
        for i in range(cls.no_of_rounds) :
            ...
            left_half = format(Operations.circular_left_shift(int(left_half, 2), cls.key_shift_values[i%2], int(cls.key_size/2)), \
                                "0{}b".format(int(cls.key_size/2)))
            right_half = format(Operations.circular_left_shift(int(right_half, 2), cls.key_shift_values[i%2], int(cls.key_size/2)), \
                                "0{}b".format(int(cls.key_size/2)))
            
            # Merge the left and right half
            merged_halfs = left_half + right_half
            # print("Left Key : " + left_half)
            # print("Right Key : " + right_half)
            # print("Full Key : " + merged_halfs);
    ...
    def encrypt(cls, plaintext, keys) :
        ...
        for key in keys :
            ...
            for block in permuted_plaintext :
                ...
                # Expand right_half menjadi 8 bit sesuai dengan key-nya
                right_half = cls.__get_permuted_value(right_half, cls.plaintext_expansion_permutation)
                # print("expand : " + right_half)
                
                # Merubah right half dan key menjadi bilangan desimal kemudian dilakukan XOR
                temp = Operations.xor_operation(int(right_half, 2), int(key, 2))
                
                # Mengembalikan bilangan yang sudah di XOR ke string biner kembali
                ...
                # Operasi XOR untuk mendapatkan right_half baru
                ...
                if (k == len(keys) - 1) :
                    ...
                    # print("new_left_half :", new_left_half)
                    # print("new_right_half :", new_right_half)
                    # print('Permuted plaintext (k=1) : ' + permuted_plaintext[i])
                else :
                    permuted_plaintext[i] = new_left_half + new_right_half
                    # print("new_left_half :", new_left_half)
                    # print("new_right_half :", new_right_half)
                    # print('Permuted plaintext (k=0) : ' + permuted_plaintext[i])
                i += 1
                
            k += 1
            print('All Permuted Plaintext After Substitution : ')
            print(permuted_plaintext)
